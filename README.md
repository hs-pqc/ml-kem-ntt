...
단발성 연산에서는 Barrett가 유리.

* 측정 환경: Docker gcc:latest, -O2, CLOCK_MONOTONIC

이제 README 전체 완성본 줄게요. 위에서 작성한 내용에 이 수치 반영해서 최종본으로:
markdown# ML-KEM NTT Implementation (FIPS 203)
...
이유: Barrett는 64-bit 정수 나눗셈 근사를 매 계수마다 수행.
Montgomery는 16-bit 곱셈으로 reduction을 대체하고
int16_t 연산으로 메모리 대역폭도 절반으로 줄어듦.
단, Montgomery는 사전에 Montgomery form 변환 오버헤드가 있어서
